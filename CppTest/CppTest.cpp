// CppTest.cpp : This file contains the 'main' function. Program execution
...
void func1() {
  ...
  dueTime.QuadPart = 0;
  BOOL result =
      SetWaitableTimer(hWaitableTimer, &dueTime, 5000, NULL, NULL, FALSE);
  if (!result) {
  ...
  while (!error) {
    i++;
    HANDLE handles[] = {hWaitableTimer, oAddrChange.hEvent,
                        oRouteChange.hEvent};
    BOOL connectivity;
    std::cout << i << " Starting to wait" << std::endl;
    DWORD waitResult =
        WaitForMultipleObjectsEx(3, handles, FALSE, INFINITE, TRUE);
    ...
    case WAIT_OBJECT_0:
      std::cout
          << i
          << " internet interval reached, checking for internet connectivity"
          << std::endl;
...
void __stdcall DnsQueryCompletionRoutine(PVOID pQueryContext,
...
BOOL CheckInternetConnectivity() {
  ...
  PDNS_QUERY_RESULT pResult =
      (PDNS_QUERY_RESULT)malloc(sizeof(DNS_QUERY_RESULT));
  ZeroMemory(pResult, sizeof(DNS_QUERY_RESULT));
  pResult->Version = DNS_QUERY_REQUEST_VERSION1;

  PDNS_QUERY_CANCEL pCancel =
      (PDNS_QUERY_CANCEL)malloc(sizeof(DNS_QUERY_CANCEL));
...
void func2() {
  for (std::string uriString; std::getline(std::cin, uriString);
       uriString != "") {
    ...
    auto begin =
        std::sregex_iterator(uriString.begin(), uriString.end(), schemeRegex);
